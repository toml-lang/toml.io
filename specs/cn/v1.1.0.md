...
- TOML 是大小写敏感的。
- 空白是指制表符（U+0009）或空格（U+0020）。
- 换行是指 LF（U+000A）或 CRLF（U+000D U+000A）。
- TOML 文件必须是合法的 UTF-8 编码的 Unicode 文档。

具体来说，这意味着文件整体必须构成一个[规范的代码单元序列](https://unicode.org/glossary/#well_formed_code_unit_sequence)。否则，必须拒绝该文件（推荐做法），或根据 Unicode 规范，将格式错误的字节序列替换为 U+FFFD。

## 注释

井号（#）将该行余下的部分标记为注释，除非它在字符串中。  
...
注释应该被用于文件的人类读者之间的交流。解析器不得基于注释的存在（或其内容）来修改键或值。
...
裸键不能为空，但空引号键是允许的（虽然不建议如此）。不能使用多行字符串来定义带引号的键。
...
>注：\xHH 表示两位十六进制，\uHHHH 表示四位十六进制，\UHHHHHHHH 表示八位十六进制

任何 Unicode 字符都可以用 `\xHH` 、`\uHHHH` 或 `\UHHHHHHHH` 的形式来转义。  
转义码必须是有效的 Unicode [标量值](https://unicode.org/glossary/#unicode_scalar_value)。  

请记住，所有 TOML 字符串都是 Unicode 字符序列，*不是*字节序列。对于二进制数据，
应避免使用这些转义码。相反，建议使用外部二进制到文本的编码策略，如十六进制序列或 
[Base64](https://www.base64decode.org/)，在字节和字符串之间进行转换。
...
任何 Unicode 字符都可以使用，除了那些必须被转义的：反斜杠和除制表符、换行符、回车符外的控制字符（U+0000 至 U+0008，U+000B，U+000C，U+000E 至 U+001F，U+007F）。回车符（U+000D）仅允许作为换行序列的一部分。
...
紧随开始标记的那个换行会被剔除。  
TOML 解析器必须以与多行基本字符串相同的方式规范化换行符。
...
```
实现可以自由支持任何大小的整数。
建议至少接受并无损处理 64 位有符号整数（从 -2^63 到 2^63-1）。
...
实现可以自由支持任何精度级别。建议至少支持 IEEE 754 binary64 值。
...
允许一个与 RFC 3339 不同的例外：可以省略秒，在这种情况下将假定为 :00。偏移量紧跟在分钟之后。
...
实现必须支持至少毫秒精度。可以指定更高精度的数字，但如果它们超过支持的精度，则必须截断多余的数字，而不是四舍五入。
...
## 数组
数组是由方括号包围的有序值。空白字符被忽略。元素间用逗号分隔。数组可以包含与键值对中允许的相同数据类型的值。可以混合不同类型的值。 
...
点分隔键为最后一个键部分之前的每个键部分创建并定义一个表。任何这样的表必须将其所有键值对定义在当前 `[table]` 表头下，或者如果定义在所有表头之前，则定义在根表中，或者定义在一个内联表中。
...
内联表提供了一种更为紧凑的语法来表示表。  
它们对于分组嵌套数据特别有用，否则这些数据很快就会变得冗长。  
内联表被完整地定义在花括号之中：`{` 和 `}`。  
括号中，可以出现零或更多个以逗号分隔的键值对。  
键值对采取与标准表中的键值对相同的形式。  
什么类型的值都可以，包括内联表。  

内联表可以在同一行上有多个键值对，也可以放在不同的行上。允许在最后一个键值对之后使用终止逗号（也称为尾随逗号）。
