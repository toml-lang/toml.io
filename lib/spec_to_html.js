...
const LATEST_TEMPLATE_PATH = path.join(HTML_DIR, "latest.html.template");
const SPEC_TEMPLATE_PATH = path.join(HTML_DIR, "spec.html.template");
const NAV_TEMPLATE_PATH = path.join(HTML_DIR, "_nav.html.template");

const SPEC_TEMPLATE = fs.readFileSync(SPEC_TEMPLATE_PATH).toString()
const NAV_TEMPLATE = fs.readFileSync(NAV_TEMPLATE_PATH).toString()
const LATEST_TEMPLATE =  fs.readFileSync(LATEST_TEMPLATE_PATH).toString()
...
class SpecToHtml {
  ...
  createSpec(locale, specFilename) {
    if (specFilename.match(/\.md$/) && !specFilename.match(/^_/)) {
      ...
      const specHtml = Formatter.format(marked(specRaw));
  ...
  generateNav() {
    ...
    // languages
    const locationsList = [];
    for (let locale in this.generatedSpecs.reverse()) {
