...
const classes = {
  header: "text-2xl font-semibold text-orange-800 pt-16",
  paragraph: "my-4",
  code: "code",
  list: "list-disc list-inside my-4",
  navItem: "text-orange-600 mt-2"
};
...
class Formatter {
  ...
  static styleCode(text) {
    return text.replace(
      /<code class="(.*?)">/g,
      `<code class="${classes.code} $1" data-controller="snippet">`
    );
  }
  ...
  static extract(text) {
    ...
    return lines
      .map(line => {
        ...
        if (match) {
          return `<li class="${classes.navItem}"><a href="#${match[1]}">${match[2]}</a></li>`;
        }
      })
      .filter(nav => nav);
  }
